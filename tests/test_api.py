...
class TestAPI:
    ...
    # New tests for web interface endpoint
    def test_web_detect_endpoint_no_file(self, client):
        """Test web detect endpoint without file."""
        response = client.post('/detect')
        assert response.status_code == 400
        assert b'No file part' in response.data

    def test_web_detect_endpoint_empty_file(self, client):
        """Test web detect endpoint with empty file."""
        data = {}
        data['file'] = (BytesIO(b''), '')
    ...
    def test_web_detect_endpoint_invalid_file(self, client):
        """Test web detect endpoint with invalid file type."""
        data = {}
        data['file'] = (BytesIO(b'not an image'), 'test.txt')
        response = client.post('/detect', data=data)
        assert response.status_code == 400
    ...
    def test_web_detect_endpoint_success(self, client, sample_image):
        """Test successful detection through web interface."""
        mock_results = [
            {
                'class': 'person',
                'confidence': 0.95,
    ...
    def test_web_detect_endpoint_large_file(self, client):
        """Test web detect endpoint with file exceeding size limit."""
        # Create a large file (17MB)
        large_data = b'0' * (17 * 1024 * 1024)
        data = {}
        data['file'] = (BytesIO(large_data), 'large.png')
